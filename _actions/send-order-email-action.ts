...
export async function sendOrderEmail(formData: FormData): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    const addressLine1 = DOMPurify.sanitize(formData.get("address-line1") as string);
    const addressLine2 = formData.get("address-line2") ? DOMPurify.sanitize(formData.get("address-line2") as string) : "";
    const city = DOMPurify.sanitize(formData.get("address-level2") as string);
    const province = DOMPurify.sanitize(formData.get("address-level1") as string);
    const postalCode = DOMPurify.sanitize(formData.get("postal-code") as string);
    const notes = formData.get("notes") ? DOMPurify.sanitize(formData.get("notes") as string) : "";
